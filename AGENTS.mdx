...
- `tenant_metadata_schema` is effectively planned up front. If you need to change filterable metadata fields, expect to create a new database or re-ingest under a new schema.
...
| `GET /databases/stats?database=...` | collection counts and vector dimensions |
...
| `403` | authentication or authorization failure (missing/malformed header, invalid/revoked/unknown key, or not permitted) | No |
...
| `FORBIDDEN` | missing/malformed bearer token, invalid/revoked/unknown key, or key lacks access |
