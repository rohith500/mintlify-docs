...
# B2C pattern: each customer gets their own collection = their user_name.
# Collections are created automatically on first write - no setup needed.
def customer_sub_tenant(customer_id: str) -> str:
    return f"customer-{customer_id}"

# Shared collection for knowledge base (visible to all agents, all customers)
...
// Create the shared database
await client.tenants.create({ database: TENANT_ID });
...
Output:
```
✓ Database 'customer-support' ready.
```
...
def ingest_help_docs(articles: list) -> list:
    ...
    for article in articles:
        batch.append({
            "id":            article["id"],
            "database":     TENANT_ID,        # required inside each item
            "collection": KB_SUB_TENANT,    # required inside each item
            "title":         article["title"],
            "type":          "confluence",
...
def _upload_kb_batch(batch: list) -> list:
    # database / collection required as top-level SDK params AND inside each item
    result = client.context.ingest(
        database=TENANT_ID,
        collection=KB_SUB_TENANT,
        upsert=True,
...
def store_conversation_turn(
    ...
    client.context.ingest(
        type='memory',
        database=TENANT_ID,
        collection=customer_sub_tenant(customer_id),
        upsert=True,
        memories=[{
            "text":      text,
...
def recall_customer_context(
    ...
    collection scopes each call to the right data store.
    """
...
|--------|----------|---------|
| `POST` | `/databases` | Create the support database |
...
client.context.ingest(
    ...
    app_knowledge=json.dumps([{
        "id":            "kb-article-001",
        "database":     "customer-support",    # also required inside each item
        "collection": "knowledge-base",      # also required inside each item
...
├── setup.py                      # database creation + SDK client init
