...
graph LR
    ...
    B -->|"POST /context/ingest<br/>multipart form-data"| C["HydraDB<br/>database: competitive-intel<br/>collection: acme-corp"]
...
|---|---|---|
| `database` | Text | `competitive-intel` |
| `collection` | Text | `acme-corp` |
| `documents` | File | your `.txt` or `.pdf` file |

### Verify Processing (query params)
```
GET /context/status?database=competitive-intel&ids=YOUR_ID
```
