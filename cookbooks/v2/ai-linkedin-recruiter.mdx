...
Use HydraDB metadata filters for exact hard requirements (for example `job_search_status: "open"`). Keep range requirements such as years of experience or salary in the natural-language query and in your own ranking step; `metadata_filters` are exact-match constraints, not range operators.
...
class AdvancedCandidateSearch extends PersonalizedRecruitingSearch {
  async searchWithComplexCriteria(query, criteria = {}) {
    ...
    // Do not use metadata_filters for ranges; rank those client-side below.
