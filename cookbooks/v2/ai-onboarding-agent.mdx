...
```mermaid
graph LR
    A["Decision logs<br/>Org charts<br/>Meeting notes<br/>Product specs"] -->|"context.ingest()"| B["HydraDB<br/>database: onboarding<br/>collection: company-context"]
    C["New hire question"] -->|"query()"| B
    B -->|"retrieved context"| D["Onboarding agent"]
    D -->|"context.ingest(type=memory)"| E["HydraDB<br/>collection: hire-{id}"]
...
DATABASE_ID = os.environ.get("HYDRADB_TENANT_ID", "onboarding")
```
...
One database holds all onboarding content. The free plan supports one database - if you already have one, skip creation and reuse it.
...
def create_tenant():
    try:
        result = client.tenants.create(database=DATABASE_ID)
...
Output:
```
Database created: {'database': 'onboarding', 'status': 'created'}
Status: Database infrastructure is ready
  scheduler=running
  graph=running
  vectorstore=running
```
...
**If it fails:** `403 Plan limit reached` - you already have a database. Skip this step and use your existing `DATABASE_ID`.
...
| `403 Plan limit reached` | Free plan allows 1 database only | Skip `create_tenant()` - reuse your existing database ID. |
...
| Free plan | 1 database maximum. All cookbooks share the same database - use different `doc_type` metadata to distinguish them. |
...
| Method | Purpose |
|---|---|
| `client.tenants.create(database=...)` | Create the onboarding database |
| `client.tenants.status(database=...)` | Check database is ready |
| `client.context.ingest(database=..., documents=..., document_metadata=...)` | Upload company knowledge documents |
| `client.context.ingest(type='memory', database=..., collection=..., memories=[...])` | Store per-hire memory |
