...
> ❌ **Authorization header must be exact.** `Authorization: Bearer hdb_abc123` - capital B, one space, no quotes. Anything else returns 403.
...
def create_tenant():
    client.tenants.create(database=DATABASE_ID)
    print("Accepted")
    _poll_until_ready()
...
def _poll_until_ready(timeout=180, interval=4):
    ...
    while time.time() < deadline:
        status = client.tenants.status(database=DATABASE_ID).data
        infra = status.infra
...
**If it fails:** `403` - API key wrong or missing `Bearer ` prefix. Confirm `.env` has `HYDRA_DB_API_KEY=hdb_abc123` with no surrounding quotes.
...
// collection is NOT required for this working flow.
...
def search(query: str, max_results: int = 10):
    ...
    Only database + query + max_results are required.
    collection is NOT required - HydraDB handles scope internally.
...
|---|---|
| Database creation | Better metadata and chunk quality |
...
| `chunk_uuid` | optional | Unique identifier for this chunk. Useful for deduplication when fan-out search across collections returns the same chunk more than once. |
...
def tuned_recall(query: str, scope: str = None, max_results: int = 15):
    """scope: optional collection. Omit to search all collections automatically."""
    kwargs = {
        "database":     DATABASE_ID,
        "query":         query,
        "max_results":   max_results,
    ...
    if scope: kwargs["collection"] = scope
...
def search_docs(
    ...
    Calls /query. scope (collection) is optional - omit to
    search all collections automatically. Response contains:
...
def recall_context(query: str, max_results: int = 10):
    ...
    scope=None means HydraDB searches all collections automatically.
    Returns SDK object with .chunks and .sources attributes.
    """
    return search_docs(
        query=query,
        max_results=max_results,
        scope=None,            # no collection needed - HydraDB handles scope
        graph_context=True,
        recency_bias=0.2,
    )
...
**Cause:** The Authorization header is missing or malformed, or the API key is invalid. Most common: the `Bearer ` prefix is absent, quotes surround the key in `.env`, or there is a leading/trailing space in the key value.
...
```json
Request:  { "database": "engineering-docs" }
...
GET /databases/status?database=engineering-docs
→ { "status": "ready" }
```
...
```json
// Required: ?ids=<id>&database=engineering-docs
