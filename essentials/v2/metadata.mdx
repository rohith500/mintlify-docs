...
### List filters and comma-joined values

When a metadata field is ingested as a list, HydraDB stores it as a comma-joined string  -  `["finance", "legal"]` becomes `"finance,legal"`. A list-valued filter still matches that stored value: the two match when they hold the same set of values, in any order. So `metadata_filters: { additional_metadata: { topics: ["legal", "finance"] } }` matches a source stored with `topics: "finance,legal"`.

Matching is full-set equality, not subset. Every value in the filter must be present in the stored value, and both sets must be the same size. A filter of `["finance"]` does not match a stored `"finance,legal"`, and a filter of `["finance", "legal"]` does not match a stored `"finance,legal,tax"`.
...
| List filter returns nothing against a list-valued field | `metadata_filters` matches lists by full-set equality, not subset | Pass the complete set of stored values, in any order. A partial list (`["finance"]` against a stored `"finance,legal"`) is treated as a mismatch. |
