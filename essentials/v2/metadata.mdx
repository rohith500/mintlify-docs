...
| Field | Type | Purpose |
| --- | --- | --- |
| `metadata` | object for knowledge; JSON-encoded string for memory items | Database-schema fields. Keys must match `tenant_metadata_schema`. The fast scoping path. |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| `metadata_filters` doesn't scope results | Key isn't declared in `tenant_metadata_schema`, or `enable_match` is `false` | Re-create the database with the field declared (`enable_match: true`), or move the field to `additional_metadata` and nest the scope under `additional_metadata`. |
...
| Tenant creation fails with "reserved" or "invalid field name" error | Field name uses a reserved system name or invalid characters | Use a different field name. See [Schema validation constraints](#schema-validation-constraints) for the full list of reserved names and naming rules. |
...
- [Create Database  -  API Reference](/api-reference/v2/endpoint/create-tenant)  -  full `tenant_metadata_schema` reference
