...
**At ingestion**, HydraDB extracts relationships from your data and stores them in the graph. You have two additional options for relationship data:

- **Forceful relations.** Declare explicit relationships to other sources via a `relations` payload.
- **Custom graphs.** Supply your own entities and relations via `graph_payload` to replace automatic extraction entirely. See [Custom graph ingestion](/api-reference/v2/endpoint/ingest-context#custom-graph-ingestion).
