...
**At ingestion**, HydraDB extracts relationships from your data and stores them in the graph. Two payloads give you more control over relationship data:

- **Forceful relations.** Declare explicit relationships to other sources via a `relations` payload.
- **Custom graphs.** Supply your own entities and relations for a source via `graph_payload` to replace automatic extraction for that source. This works for documents, app sources, and memories. See [Custom graph ingestion](/api-reference/v2/endpoint/ingest-context#custom-graph-ingestion).
