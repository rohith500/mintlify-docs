...
<Warning>
**Delete failures leave the tenant intact.** Deletion starts by removing the tenant's ID mapping. If that step fails, the endpoint returns `500` and no background cleanup is scheduled. The tenant stays registered and its data is left untouched, so the request is safe to retry.
</Warning>
