...
<Warning>
**Mapping delete failures.** Deletion starts by removing the tenant's ID mapping. If that step fails, the endpoint returns `500` with `Failed to delete tenant mapping; tenant was not deleted. Retry the request.` The tenant stays registered and no background cleanup is scheduled, so its data is left intact. The request is safe to retry.
</Warning>
...
Common codes: `400 INVALID_PARAMETERS`, `404 TENANT_NOT_FOUND`, `500 INTERNAL_ERROR`. See [Error Responses](/api-reference/error-responses) for the full list.
