...
<Note>
Recall endpoints may enforce per-API-key rate limits. When exceeded, the API returns HTTP 429 with a message indicating the limit (e.g., "Query rate limit exceeded: max 120 requests per minute"). Use exponential backoff for retries.
</Note>
