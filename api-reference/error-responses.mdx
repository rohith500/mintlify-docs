...
| `401` | Unauthorized | Invalid or unknown API key |
| `403` | Forbidden | Missing or malformed `Authorization` header, or API key lacks permission |
...
The `Authorization` header is missing or malformed, or the API key lacks permission for the requested resource.
