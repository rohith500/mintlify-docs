...
- **General rate limiting:** All API endpoints may return 429 under sustained load.
- **Query rate limiting:** Recall endpoints (`/recall/full_recall`, `/recall/recall_preferences`, `/recall/boolean_recall`) may enforce a per-API-key request rate limit. When exceeded, the response includes a specific message:

```json
{
  "detail": "Query rate limit exceeded: max 120 requests per minute. Please retry shortly."
}
```

**How to handle:**

For query rate limit errors, check the `Retry-After` response header. When present, it indicates the number of seconds until the rate limit window resets. Prefer waiting the `Retry-After` duration before retrying.

For general rate limiting without a `Retry-After` header, implement exponential backoff. See [Retry pattern](#retry-pattern) below.
