...
| `403` | Forbidden | Authentication or authorization failure (missing/malformed `Authorization` header, invalid/revoked/unknown API key, or insufficient permission) |
...
HydraDB returns `403` for any authentication or authorization failure: a missing or malformed `Authorization` header, an invalid, revoked, or unrecognized API key, or a valid key that lacks permission for the requested resource.
...
- Invalid, unrecognized, or revoked API key
  ...
  if (error instanceof HydraDBError) {
    switch (error.body?.detail?.error_code) {
      ...
      case "RATE_LIMITED":
        ...
        break;
      case "FORBIDDEN":
        // Check the Authorization header and API key
        break;
      default:
