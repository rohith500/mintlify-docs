...
<AccordionGroup>
  ...
  <Accordion title="Filters">
    ...
    |---|---|
    | **Top-level keys** (`department`, `region`) | The source's schema-backed `metadata`. Keys must be declared in the database's `tenant_metadata_schema` with `enable_match: true`, otherwise they are silently ignored. |
...
- **Use the right metadata namespace.** Top-level `metadata_filters` keys match `metadata`; free-form per-document fields must be nested under `additional_metadata` (`document_metadata` is only a legacy alias). Declare hot top-level filter fields in `tenant_metadata_schema` with `enable_match: true`.
...
Common codes: `400 INVALID_PARAMETERS` (empty `query`), `404 DATABASE_NOT_FOUND`, `422 VALIDATION_ERROR`, `429 RATE_LIMITED`, `500 INTERNAL_ERROR`. See [Error Responses](/api-reference/v2/error-responses) for the full list.

<Note>
**Query rate limiting is off by default.** When an operator enables it, the query endpoint enforces a per-API-key request limit. Exceeding the limit returns HTTP `429` with a `Retry-After` header giving the number of seconds to wait before retrying. Honor `Retry-After` when it is present, and otherwise fall back to exponential backoff.
</Note>
