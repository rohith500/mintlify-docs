...
<Note>
Query endpoints may enforce per-API-key rate limits. When exceeded, the API returns HTTP 429 with a `Retry-After` header indicating the seconds until the rate limit window resets (e.g., "Query rate limit exceeded: max 120 requests per minute"). Wait for the `Retry-After` duration or use exponential backoff for retries.
</Note>
