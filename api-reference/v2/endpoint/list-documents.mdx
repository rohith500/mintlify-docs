...
| --- | --- | --- |
| <Field name="metadata" type="object" /> | Context item's schema-aligned `metadata` payload | Use for tenant metadata fields. `tenant_metadata` is accepted as a legacy alias. Declare hot filter fields in `tenant_metadata_schema` with `enable_match: true`. |
