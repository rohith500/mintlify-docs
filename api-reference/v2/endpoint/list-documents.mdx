...
| --- | --- | --- |
| <Field name="metadata" type="object" /> | Context item's schema-aligned `metadata` payload | Use for database metadata fields. `tenant_metadata` is accepted as a legacy alias. Keys must be declared in the database's `tenant_metadata_schema` with `enable_match: true`; undeclared keys are silently ignored. |
...
When you don't need every field on every row, pass `include_fields` to keep response payloads small. Only the listed display and metadata fields are populated; treat any omitted field as unavailable in that response. `id`, `database`, and `collection` are always returned.
