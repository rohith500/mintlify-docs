...
| <Field name="graph_payload" type="string (JSON object)" /> | Optional custom knowledge graph to attach to the document, **knowledge only**. When provided, HydraDB uses your graph instead of extracting one automatically. Requires exactly one document and cannot be combined with `app_knowledge`. See [Custom graph ingestion](#custom-graph-ingestion) below. |
...
Use `graph_payload` to supply your own knowledge graph instead of relying on automatic extraction. Helpful when you have pre-existing structured relationships or need precise control over entity and relation representation.

<Accordion title="5. Custom graph - supply your own entities and relations">
  <CodeGroup>

  ```python Python SDK
  import json

  with open("/path/to/document.pdf", "rb") as doc:
      result = client.context.ingest(
          type="knowledge",
          tenant_id="acme_corp",
          sub_tenant_id="team_docs",
          documents=[("document.pdf", doc, "application/pdf")],
          graph_payload=json.dumps({
              "entities": {
                  "e1": {"name": "John Doe", "type": "PERSON", "namespace": "employees"},
                  "e2": {"name": "Acme Corp", "type": "ORGANIZATION", "namespace": "organizations"}
              },
              "relations": [
                  {
                      "source": "e1",
                      "target": "e2",
                      "predicate": "WORKS_FOR",
                      "context": "John Doe leads the payments team at Acme Corp."
                  }
              ]
          }),
      )
  ...
  const result = await client.context.ingest({
    ...
    subTenantId: "team_docs",
    documents: [
      { path: "/path/to/document.pdf", filename: "document.pdf", contentType: "application/pdf" },
    ],
  ...
  | Field | Description |
  | --- | --- |
  | <Field name="entities" type="object" required /> | Map of entity keys to entity objects. Keys are local identifiers used to reference entities in `relations`. |
  | <Field name="relations" type="array" required /> | Array of relation objects connecting entities. |

  ### Entity object

  | Field | Description |
  | --- | --- |
  | <Field name="name" type="string" required /> | Entity name. Max 256 characters. |
  ...
  | <Field name="identifier" type="string" /> | Optional external identifier (e.g., a URL or database ID). |

  ### Relation object

  | Field | Description |
  | --- | --- |
  | <Field name="source" type="string" required /> | Key of the source entity in the `entities` map. |
  | <Field name="target" type="string" required /> | Key of the target entity in the `entities` map. |
  | <Field name="predicate" type="string" required /> | Relationship type (e.g., `"WORKS_FOR"`, `"MENTORS"`, `"DEPENDS_ON"`). Max 256 characters. |
  | <Field name="context" type="string" /> | Contextual description of the relationship. Max 2000 characters. |

  ### Constraints

  - **Single document only.** `graph_payload` requires exactly one document. Multi-document uploads are not supported.
  - **Cannot combine with `app_knowledge`.** Use either `graph_payload` with `documents` or `app_knowledge` alone.
  - **Non-empty graph required.** Both `entities` and `relations` must have at least one entry.
  - **Referential integrity.** All `source` and `target` values in relations must reference keys declared in `entities`.
  ...
  | --- | --- |
  | Max entities | 5,000 |
  | Max relations | 10,000 |
  | Max relations per entity | 500 |
  | Entity name/type/namespace max length | 256 characters |
  | Predicate max length | 256 characters |
  | Context max length | 2,000 characters |

  ### Behavior

  - The supplied graph **replaces** automatic extraction for this document.
  - The graph is stored durably and survives document re-ingestion—if you re-upload the same document without `graph_payload`, the original graph remains intact.
  - Graph relationships surface in query responses when `graph_context=true`.
</Accordion>
