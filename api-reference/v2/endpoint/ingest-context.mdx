...
- **Declare hot schema fields upfront.** Put frequently filtered fields in `metadata`, define them in `tenant_metadata_schema` with `enable_match: true`, and use `additional_metadata` for free-form display/bookkeeping fields. Additive schema updates exist, but newly added dense/sparse metadata lanes are not backfilled into existing Milvus collections.
