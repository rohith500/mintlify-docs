...
<RequestExample>
```python Python SDK
response = client.databases.delete(database="my_first_database")
```
...
```
```bash cURL
curl -X DELETE 'https://api.hydradb.com/databases?database=my_first_database' \
...
```json Success
{
  ...
  "data": {
    ...
    "message": "Database deregistered. Background cleanup is in progress."
  },
  "error": null,
  "meta": {
...
## Mapping delete failures

Deletion starts by removing the tenant's ID mapping. If that step fails, HydraDB returns `500` and skips background cleanup: the tenant stays registered, continues to appear in `GET /tenants`, and its vector stores, graphs, and storage objects are left intact. This is deliberate  -  failing here avoids destroying a tenant's data while a mapping that still points to it remains. The request is safe to retry.

## Errors

Common codes: `404 DATABASE_NOT_FOUND`, `401 UNAUTHORIZED`, `422 VALIDATION_ERROR`, `500 INTERNAL_ERROR`. See [Error Responses](/api-reference/v2/error-responses) for the full list.
...
- **Alternative:** [Delete Context](/api-reference/v2/endpoint/delete-source) - remove specific knowledge or memories without deleting the whole database
