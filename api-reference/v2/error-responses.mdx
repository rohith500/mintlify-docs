...
| `401` | Invalid or unknown API key | No |
| `403` | Missing or malformed `Authorization` header, or API key lacks permission | No |
...
| `UNAUTHORIZED` | `401` | The API key is invalid or unrecognized. |
| `FORBIDDEN` | `403` | The `Authorization` header is missing or malformed, or the API key lacks permission for the requested resource. |
