...
| `429` | Rate limit exceeded | Yes, with backoff (check `Retry-After` header) |
...
| `RATE_LIMITED` | `429` | The API key exceeded its current rate limit. The query endpoint (`/query`) may enforce a per-API-key request limit when query rate limiting is enabled. |
...
For `429` responses from query endpoints, check the `Retry-After` response header. When present, it indicates the number of seconds until the rate limit window resets. Prefer waiting the `Retry-After` duration before retrying.
