...
| `403` | Authentication or authorization failure: missing/malformed `Authorization` header, invalid/revoked/unknown API key, or API key lacks permission | No |
...
| `FORBIDDEN` | `403` | Authentication or authorization failure: the `Authorization` header is missing or malformed, the API key is invalid, revoked, or unrecognized, or the key lacks permission for the requested resource. |
