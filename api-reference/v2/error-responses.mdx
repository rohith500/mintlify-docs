...
| `401` | Missing or malformed `Authorization` header | No |
| `403` | Invalid, revoked, expired, or unauthorized API key | No |
...
| `UNAUTHORIZED` | `401` | The `Authorization` header is missing or malformed. |
| `FORBIDDEN` | `403` | The API key is invalid, revoked, expired, or not authorized for the requested resource. |
