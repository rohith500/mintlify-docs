...
| `RATE_LIMITED` | `429` | The API key exceeded its current rate limit. Query endpoints (`/query`, `/recall/*`) may return a per-minute rate limit message when query rate limiting is active. |
