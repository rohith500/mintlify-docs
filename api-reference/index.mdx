...
- **Authentication:** Every endpoint requires `Authorization: Bearer $HYDRA_DB_API_KEY`
...
**Authentication.** Every endpoint requires `Authorization: Bearer $HYDRA_DB_API_KEY` in the request header (substitute the actual key value). Get your key at [app.hydradb.com](https://app.hydradb.com).
...
- **Go deeper:** [Essentials](/essentials/architecture) covers each primitive in depth
